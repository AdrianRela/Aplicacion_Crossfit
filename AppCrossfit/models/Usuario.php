...
class Usuario extends ActiveRecord {
    ...
    public function validarNuevaCuenta() {
        $patronTel = "/^[679]\d{8}$/";
        $patronEmail = '/^[A-Za-z0-9._%+-]+@(gmail|hotmail)\.(com|es)$/';
        ...
        if (!preg_match($patronTel, $this->telefono)) {
            self::$alertas['error'][] = "Número Incorrecto, debe tener 9 números y comenzar por 6, 7 o 9";
        }
        if(!$this->email) {
            self::$alertas ['error'][] = 'El Email es Obligatorio';
        }
        if (!preg_match($patronEmail, $this->email)) {
            
            self::$alertas ['error'][] = 'El Email es Incorrecto, debe ser gmail o hotmail .com o .es';
        }
        if(!$this->password) {
